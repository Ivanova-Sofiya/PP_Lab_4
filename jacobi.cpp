...
double to_real(double x, double step, int n)
{
	return x + step * n;
}
...
void setGrid(double***& grid, int size_x, double start_x, int num_rank, int proc_count)
{
	...
	// Задаём начальное приближение для НЕ краевых узлов 
	...
	// Задаём краевые значения для плоскостей XY и XZ

	for (int i = 0; i < size_x; i++) {
		double current_x = to_real(start_x, hx, i);

		// обрабатываем верхнюю и нижнию плоскости XY подобласти
		for (int k = 0; k < Nz; k++) {
			// При Z = 0
			grid[i][0][k] = phi_func(current_x, Y1, to_real(Z1, hz, k));
			// При Z = Ny - 1
			grid[i][Ny - 1][k] = phi_func(current_x, Y2, to_real(Z1, hz, k));
		...
		for (int j = 1; j < Ny - 1; j++) {
			...
			double current_y = to_real(Y1, hy, j);
			// При Z = 0
	...
	if (num_rank == 0) {
		...
		for (int j = 1; j < Ny - 1; j++) {
			for (int k = 1; k < Nz - 1; k++) {
				grid[0][j][k] = phi_func(X1, to_real(Y1, hy, j), to_real(Z1, hz, k));
			}
		}
	}
	...
	if (num_rank == proc_count - 1) 
	{
		for (int j = 1; j < Ny - 1; j++) {
			for (int k = 1; k < Nz - 1; k++) {
				grid[size_x - 1][j][k] = phi_func(X2, to_real(Y1, hy, j), to_real(Z1, hz, k));
			}
		}
	}
	...
	for (int i = 1; i < size_x - 1; i++) {
		for (int j = 1; j < Ny - 1; j++) {
			for (int k = 1; k < Nz - 1; k++) {
				...
				err = abs(grid[i][j][k] - phi_func(to_real(start_x, hx, i), to_real(Y1, hy, j), to_real(Z1, hz, k)));
...
void Jacobi(double***& grid_1, int size_x, int start_x, int proc_lower, int proc_upper)
{
	...
	double converg;			// значение сходимости на узле
	double maxConverg;		// маскимальное значение сходимости в процессе на итерации
	char isEnd;				// вспомогательный флаг
	...
	for (int i = 0; i < size_x; i++)
{
	...
	double*** present = grid_1;	// указатель на текущие значения массива
	double*** past = grid_2;		// указатель на значения массива на предыдущей итерации
	double*** help;				// указатель для перемены указателей на массивы

	int messageLength = (Ny - 2) * (Nz - 2);		// размер буфера сообщения (при обмене информацией)
	double* messageBuf = new double[messageLength]; // буфер сообщения (при обмене информацией)
	...
	while (flag) {
		...
		for (int j = 1; j < Ny - 1; j++) {
			for (int k = 1; k < Nz - 1; k++) {
				...
				past[1][j][k] = (present[2][j][k] + present[0][j][k]) / hX2;
				past[1][j][k] += (present[1][j + 1][k] + present[1][j - 1][k]) / hY2;
				past[1][j][k] += (present[1][j][k + 1] + present[1][j][k - 1]) / hZ2;
				past[1][j][k] -= rho(present[1][j][k]);
				past[1][j][k] *= c;

				// Вычисляем сходимость для данного узла
				converg = abs(past[1][j][k] - present[1][j][k]);
				if (converg > maxConverg) maxConverg = converg;
			}
		}
		...
		if (proc_lower != -1) {
			for (int j = 0; j < Ny - 2; j++) {
				for (int k = 0; k < Nz - 2; k++) {
					messageBuf[(Ny - 2) * j + k] = past[1][j + 1][k + 1];
				}
			}
			MPI_Send((void*)messageBuf, messageLength, MPI_DOUBLE, proc_lower, msg_tag6, MPI_COMM_WORLD);
		}
		...
		if (size_x != 3) {
			...
			for (int j = 1; j < Ny - 1; j++) {
				for (int k = 1; k < Nz - 1; k++) {
					...
					// Вычисляем значения на узле (формула 3 методических указаний)
					past[size_x - 2][j][k] = (present[size_x - 1][j][k] + present[size_x - 3][j][k]) / hX2;
					past[size_x - 2][j][k] += (present[size_x - 2][j + 1][k] + present[size_x - 2][j - 1][k]) / hY2;
					past[size_x - 2][j][k] += (present[size_x - 2][j][k + 1] + present[size_x - 2][j][k - 1]) / hZ2;
					past[size_x - 2][j][k] -= rho(present[size_x - 2][j][k]);
					past[size_x - 2][j][k] *= c;

					// Вычисляем сходимость для данного узла
					converg = abs(past[size_x - 2][j][k] - present[size_x - 2][j][k]);
					if (converg > maxConverg) {
						maxConverg = converg;
					}
				}
			}
		...
		if (proc_upper != -1) {
			...
			for (int j = 0; j < Ny - 2; j++) {
				for (int k = 0; k < Nz - 2; k++) {
					messageBuf[(Ny - 2) * j + k] = past[size_x - 2][j + 1][k + 1];
				}
			}
			MPI_Send((void*)messageBuf, messageLength, MPI_DOUBLE, proc_upper, msg_tag5, MPI_COMM_WORLD);
		}
		...
		for (int i = 2; i < size_x - 2; i++) {
			for (int j = 1; j < Ny - 1; j++) {
				for (int k = 1; k < Nz - 1; k++) {
					...
					// Вычисляем значения на узле (формула 3 методических указаний)
					past[i][j][k] = (present[i + 1][j][k] + present[i - 1][j][k]) / hX2;
					past[i][j][k] += (present[i][j + 1][k] + present[i][j - 1][k]) / hY2;
					past[i][j][k] += (present[i][j][k + 1] + present[i][j][k - 1]) / hZ2;
					past[i][j][k] -= rho(present[i][j][k]);
					past[i][j][k] *= c;

					// Вычисляем сходимость для данного узла
					converg = abs(past[i][j][k] - present[i][j][k]);
					if (converg > maxConverg) {
						maxConverg = converg;
					}
		...
		if (proc_lower != -1) {
			MPI_Recv((void*)messageBuf, messageLength, MPI_DOUBLE, proc_lower, msg_tag5, MPI_COMM_WORLD, &status);
			for (int j = 0; j < Ny - 2; j++) {
				for (int k = 0; k < Nz - 2; k++) {
					past[0][j + 1][k + 1] = messageBuf[(Ny - 2) * j + k];
				}
			}
		}
		...
		if (proc_upper != -1) {
			MPI_Recv((void*)messageBuf, messageLength, MPI_DOUBLE, proc_upper, msg_tag6, MPI_COMM_WORLD, &status);
			for (int j = 0; j < Ny - 2; j++) {
				for (int k = 0; k < Nz - 2; k++) {
					past[size_x - 1][j + 1][k + 1] = messageBuf[(Ny - 2) * j + k];
				}
			}
		}
		...
		MPI_Reduce((void*)&isEnd, (void*)&flag, 1, MPI_CHAR, MPI_BOR, 0, MPI_COMM_WORLD);
		...
		// Ожидаем начала новой итерации цикла
		MPI_Barrier(MPI_COMM_WORLD);
	}

	if (present == grid_1) deleteGrid(grid_2, size_x);
	else 
	{
		help = grid_1;
		grid_1 = present;
		deleteGrid(help, size_x);
	}
...
int main(int argc, char** argv)
{
	...
	MPI_Comm_rank(MPI_COMM_WORLD, &num_rank);		// номер текущего потока
	...
	setGrid(grid, size_x, to_real(X1, hx, proc_start_x - 1), num_rank, proc_count);
	...
	double accuracy = Get_Accuracy(grid, size_x, to_real(X1, hx, proc_start_x - 1), 0);
